...
Power.tau.table = cbind.data.frame(Power.tau,Power_of_T2,Power_of_dCov)

setwd('C:/Users/hp/Desktop/R-programs-for-gen-semipar/Paper_for_ADAMAS_CONF')
write.csv(Power.tau.table, file = 'Simulated_powers_Vn_r=10-Ex1_Robinson.csv')
plot(mu, Power_of_T2, xlim=c(mu[1],mu[length(mu)]), ylim=c(0,1), type="l", pch=10, col="black", xlab=expression(mu), ylab=expression("Power (Vn)"))
